...
commands (eg.: cat file | cut -d";" -f field_number | wc -options). 
It's recommended to oversize the observed quantities.

It's suggested to read the input file using the fgets function, using a memory space oversized relative to
